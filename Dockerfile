...
RUN mkdir -p /go/src/github.com/AniketBajpai/puppy-love
WORKDIR /go/src/github.com/AniketBajpai/puppy-love
...
FROM alpine
RUN mkdir -p /go/bin
COPY --from=builder /go/src/github.com/AniketBajpai/puppy-love/puppy-love /go/bin
